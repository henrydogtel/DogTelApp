...
export class SitterServiceRepository {
    constructor(@InjectRepository(ServicesSitter) private readonly serviceRepository: Repository<ServicesSitter>) { }


    async createService(sitter_id: string, createServicesSitterInput: CreateServicesSitterInput): Promise<ServicesSitter> {
        const { name, description } = createServicesSitterInput

        try {
            const serviceCreated = this.serviceRepository.create({ name, description });
            if (!serviceCreated) throw new BadRequestException('Error at create service')
    ...
    async findOne(id: string): Promise<ServicesSitter> {
        try {
            const serviceById = await this.serviceRepository.findOne({ where: { id } });
            if (!serviceById) throw new NotFoundException('the service was not found')
            return serviceById
        } catch (error) {
            throw error
        }
    ...
    async updateService(id: string, updateServicesSitterInput: UpdateServicesSitterInput): Promise<ServicesSitter> {
        try {
    ...
    async removeService(id: string): Promise<boolean> {
        try {
            const service = await this.findOne(id);
            if (!service) {
                throw new NotFoundException('The service to remove it could not be found')
            }
