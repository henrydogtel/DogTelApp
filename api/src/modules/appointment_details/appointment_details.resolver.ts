...
export class AppointmentDetailsResolver {
  constructor(
    private readonly appointmentDetailsService: AppointmentDetailsService,
  ) {}

  @Mutation(() => AppointmentDetail)
  async createAppointmentDetail(
    @Args('createAppointmentDetailInput')
    createAppointmentDetailInput: CreateAppointmentDetailInput,
  ) {
    try {
      return await this.appointmentDetailsService.create(
        createAppointmentDetailInput,
      );
    } catch (error) {
      console.error('Error creating appointment detail:', error);
      throw new Error(
        'An error occurred while creating the appointment detail. Please try again.',
      );
    }
  }
  ...
  async findAll() {
    try {
      ...
      throw new Error(
        'An error occurred while retrieving appointment details. Please try again.',
      );
    }
  }
  ...
  async findOne(@Args('id', { type: () => Int }) id: number) {
    try {
      ...
      throw new Error(
        'An error occurred while finding the appointment detail. Please try again.',
      );
    }
  }
  ...
  async updateAppointmentDetail(
    @Args('updateAppointmentDetailInput')
    updateAppointmentDetailInput: UpdateAppointmentDetailInput,
  ) {
    try {
      return await this.appointmentDetailsService.update(
        updateAppointmentDetailInput.id,
        updateAppointmentDetailInput,
      );
    } catch (error) {
      console.error(
        `Error updating appointment detail with ID ${updateAppointmentDetailInput.id}:`,
        error,
      );
      throw new Error(
        'An error occurred while updating the appointment detail. Please try again.',
      );
    }
  }
  ...
  async removeAppointmentDetail(@Args('id', { type: () => Int }) id: number) {
    try {
      ...
      throw new Error(
        'An error occurred while removing the appointment detail. Please try again.',
      );
    }
  }
}
